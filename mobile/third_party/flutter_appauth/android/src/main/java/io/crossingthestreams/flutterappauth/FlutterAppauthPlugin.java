...
public class FlutterAppauthPlugin implements FlutterPlugin, MethodCallHandler, PluginRegistry.ActivityResultListener, ActivityAware {
    ...
    // jatuh ke browser lain kalau Chrome memang tidak terinstal di perangkat.
    ...
    private static net.openid.appauth.browser.BrowserMatcher buildPreferChromeBrowserMatcher(Context context) {
        final boolean chromeInstalled = isPackageInstalled(context, CHROME_PACKAGE_NAME);
        return new net.openid.appauth.browser.BrowserMatcher() {
            @Override
            public boolean matches(net.openid.appauth.browser.BrowserDescriptor descriptor) {
                if (chromeInstalled) {
                    return CHROME_PACKAGE_NAME.equals(descriptor.packageName);
                }
                // Chrome tidak ada - terima browser apa pun yang tersedia (perilaku default AppAuth).
                return true;
            }
        };
    }
    ...
    private void onAttachedToEngine(Context context, BinaryMessenger binaryMessenger) {
        ...
        net.openid.appauth.browser.BrowserMatcher preferChrome = buildPreferChromeBrowserMatcher(context);

        AppAuthConfiguration.Builder defaultConfigBuilder = new AppAuthConfiguration.Builder();
        defaultConfigBuilder.setBrowserMatcher(preferChrome);
        defaultAuthorizationService = new AuthorizationService(this.applicationContext, defaultConfigBuilder.build());
        ...
        authConfigBuilder.setBrowserMatcher(preferChrome);
