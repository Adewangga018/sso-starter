...
import {
  ...
  X, History, Pencil, Trash2, Users, UploadCloud, ShieldCheck, Download, Layers, Building2,
} from 'lucide-react'
...
const EMPTY_FORM = {
  ...
  semuaKompartemen: true, kompartemen: [], tglBerlaku: '', ringkasan: '', file: null,
}
...
export default function ProsedurPage() {
  ...
  const load = useCallback(async (term, jn, kp) => {
    setLoading(true)
    try { setData(await api.getProsedurList(term, jn, kp)); setError('') }
    catch (err) {
  ...
  async function ack(id) {
    try { await api.ackProsedur(id); setMsg({ t: 'ok', m: 'Terima kasih, dokumen ditandai sudah Anda baca.' }); await load(q, jenis, komp); if (detail?.id === id) openDetail(id) }
  ...
  async function del(dok) {
    ...
    try { await api.hapusProsedur(dok.id); setMsg({ t: 'ok', m: 'Dokumen dihapus.' }); setDetail(null); await load(q, jenis, komp) }
    catch (err) { setMsg({ t: 'err', m: err instanceof ApiError ? err.message : 'Gagal menghapus.' }) }
  }
  async function setStatus(versiId, status) {
    try { await api.statusVersiProsedur(versiId, status); await openDetail(detail.id); await load(q, jenis, komp) }
    catch (err) { setMsg({ t: 'err', m: err instanceof ApiError ? err.message : 'Gagal.' }) }
  }
  ...
  return (
    <div className="pr">
      ...
      <div className="pr__tools">
        <form className="pr__searchwrap" onSubmit={(e) => { e.preventDefault(); load(q, jenis, komp) }}>
          <Search size={16} />
          <input className="pr__search" placeholder="Cari kode / judul / kategori / unit…" value={q} onChange={(e) => setQ(e.target.value)} />
        </form>
        <select className="pr__select" value={jenis} onChange={(e) => { setJenis(e.target.value); load(q, e.target.value, komp) }}>
          <option value="">Semua jenis</option>
          {JENIS.map((j) => <option key={j} value={j}>{j}</option>)}
        </select>
        <select className="pr__select" value={komp} onChange={(e) => { setKomp(e.target.value); load(q, jenis, e.target.value) }}>
              ...
              return (
                <article key={d.id} className={`pr__card pr__card--${color}`}>
                  ...
                  <div className="pr__card-scope">
                    ...
                    <Cakupan semua={d.semuaKompartemen} kompartemen={d.kompartemen} />
                  </div>
            ...
            {detailLoading || !detail.kode ? <div className="pr__loading"><Loader2 className="pr__spin" size={20} /> Memuat…</div> : (
              <div className="pr__modal-body">
                <div className="pr__meta">
                  ...
                  <Cakupan semua={detail.semuaKompartemen} kompartemen={detail.kompartemen} />
                      ...
                      <tr key={v.id}>
                        ...
                        <td>
                          <div className="pr__rowact">
                            <button type="button" className="pr__ibtn" title="Lihat" onClick={() => openPreview(v.id, v.namaFile, `${detail.kode} · v${v.versi}`)}><Eye size={14} /></button>
